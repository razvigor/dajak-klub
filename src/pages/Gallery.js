...
const Gallery = () => {
  ...
  useEffect(() => {
    axios.get(`http://mbportfolio.info/api/wp-json/wp/v2/posts?categories=3&_embed&per_page=10&page=1`).then(res => {
  ...
  const content = galleryCategories !== null ? galleryCategories.map(item => {
